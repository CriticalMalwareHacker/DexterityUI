...
import { SignupForm } from "@/registry/default/signup/signup";

const installCommand = `npx shadcn@latest add "https://dexterityui.vercel.app/r/signup.json"`;

const usageCode = `import { SignupForm } from "@/components/ui/signup"
...
export default function SignupPage() {
    return (
        <div className="max-w-4xl space-y-10">
            {/* Header */}
            <div>
                <h1 className="text-3xl font-bold text-white mb-2">Signup Form</h1>
                ...
                <Tabs tabs={[
                    {
                        ...
                        content: (
                            <div className="rounded-lg border border-[#1a1a1a] bg-[#111] p-10 flex items-center justify-center">
                                <SignupForm
                                    onSubmit={async ({ name, email, password }) => { console.log(name, email, password); }}
                                    onGoogleSignIn={() => { }}
                                    onSignIn={() => { }}
                                />
                            </div>
            ...
            <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[
                        { icon: "🔐", title: "Password strength meter", desc: "Live 4-bar indicator — Weak, Fair, Good, Strong" },
                        { icon: "👁️", title: "Show / hide password", desc: "Toggle password visibility without losing focus" },
                        { icon: "✅", title: "Custom checkbox", desc: "Accessible Terms & Privacy agreement checkbox" },
                        { icon: "⚡", title: "Google OAuth", desc: "One-click Google sign-up with branded button" },
                        { icon: "⏳", title: "Loading state", desc: "Submit button shows spinner during async operations" },
                        { icon: "🎨", title: "Fully composable", desc: "All handlers passed as props, zero internal state dependencies" },
                    ].map((f) => (
                        <div key={f.title} className="flex items-start gap-3 rounded-xl border border-[#1a1a1a] bg-[#111] p-4">
            ...
            <section className="space-y-4">
                ...
                <div className="rounded-xl border border-[#1a1a1a] overflow-hidden">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {[
                                { prop: "onSubmit", type: "(values) => void", desc: "Called with { name, email, password } on form submit." },
                                { prop: "onGoogleSignIn", type: "() => void", desc: "Called when the Google button is clicked." },
                                { prop: "onSignIn", type: "() => void", desc: "Called when the 'Sign in' link is clicked." },
